...
from metrics import f1_score, bad_case, predict_output, predict_output_pre
...
def train_epoch(train_loader, model, optimizer, scheduler, epoch):
    ...
    for idx, batch_samples in enumerate(tqdm(train_loader)):
        ...
        # print(batch_data, batch_token_starts, batch_labels)
        ...
        #     continue
    print('error:',error)
    train_loss = float(train_losses) / len_train_loader
...
def evaluate(dev_loader, model, mode='dev',flag=1):
...
#         print(pred_tags,sent_data)
        all_end=predict_output(pred_tags, sent_data)
    else:
